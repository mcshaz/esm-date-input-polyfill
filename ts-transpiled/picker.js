...
export class Picker {
    constructor({ yrsBack = 80, yrsFwd = 20 } = { yrsBack: 80, yrsFwd: 20 }) {
        ...
        Picker.createRangeSelect(this.year, this.date.getFullYear() - yrsBack, this.date.getFullYear() + yrsFwd);
    ...
    sync() {
        ...
        const dateVal = this.input.element.valueAsDate;
        this.date = dateVal
            ? Picker.utcDateToLocal(dateVal)
            : new Date();
    ...
    refreshLocale() {
        ...
        Picker.createRangeSelect(this.month, 0, 11, this.input.translation.months, this.date.getMonth());
    ...
    refreshDaysMatrix() {
        ...
        const daysInMonth = new Date(this.date.getFullYear(), month + 1, 0).getDate(); // Get days in month (1-31).
        // The input's current date.
        let selDate = this.input.element.valueAsDate;
        if (selDate) {
            selDate = Picker.utcDateToLocal(selDate);
        }
        ...
        for (let i = 0; i < daysInMonth + startDay; ++i) {
            ...
            matrixHTML.push(`<td class="${classList.join(' ')}">${dayNum}</td>`);
        }
    ...
    static createInstance({ yrsBack = 80, yrsFwd = 20 } = { yrsBack: 80, yrsFwd: 20 }) {
